...
namespace ClinicAppointmentSystem.Managers
{
    public class ClinicManager : ILoggable
    {
        ...
        public int AddAppointment(int patientId, int doctorId, DateTime dateTime)
        {
            ...
            Log($"Added appointment {appointmentId} for {patient.Name} with {doctor.Name}");
        ...
        public List<Appointment> SearchAppointmentsByDoctor(int doctorId)
        {
           var doctor = SearchDoctorById(doctorId);
            if (doctor == null)
                return new List<Appointment>();

            return _appointments.FindAll(a => a.Doctor.DoctorId == doctorId && a.Status == "Scheduled");
        }

        public List<Appointment> SearchAppointmentsByPatient(int patientId)
        {
           var patient = SearchPatientById(patientId);
            if (patient == null)
                return new List<Appointment>();

            return _appointments.FindAll(a => a.Patient.PatientId == patientId && a.Status == "Scheduled");
        }
